...
AI 驱动的 VMware vCenter/ESXi VM 生命周期管理与部署工具 — 49 个工具。

- **只读模式**（v1.8.0）— 一个环境变量（`VMWARE_READ_ONLY=true`）即可在启动时把 36 个具有写效果的工具（35 个写操作**外加 `vm_guest_download`**）从 MCP 注册表中移除，仅保留 13 个只读工具；适合审计、PoC 演示和不可信/本地模型场景。详见[只读模式](#只读模式)。
...
| 场景 | 推荐模式 | 原因 |
|------|:-------:|------|
| **本地/小模型**（Ollama、Qwen <32B） | **CLI** | 上下文占用 ~2K tokens vs MCP ~10K；小模型难以处理 49 个工具的 schema |
...
提示词约束只是建议——模型可以无视它。只读模式是结构性的：设置 `VMWARE_READ_ONLY=true`，全部 36 个写工具（虚拟机生命周期与电源、快照、克隆/迁移、OVA/模板/链接克隆与批量部署、Guest 操作、集群管理、Plan 与 TTL、告警确认/重置）会在启动时从 MCP 注册表中移除，仅保留 13 个读工具。`list_tools()` 根本不会列出它们——模型看不见的工具就无法调用。默认关闭；且为 fail-closed 设计：请求了只读模式但无法保证时，服务器直接拒绝启动。

`vm_guest_download` 标注为 `[READ]`——它不改变 vCenter 状态——但仍被有意归类为写工具一并移除，因为它会向调用方指定的本地路径写文件，并需要 Guest 凭据。
...
- 按 skill 覆盖：`VMWARE_AIOPS_READ_ONLY=true`（优先于家族级 `VMWARE_READ_ONLY`）
- 配置文件方式：在 `~/.vmware-aiops/config.yaml` 中设置 `read_only: true`
