...
def env(monkeypatch):
    ...
    vlan.vlanId = 100
    dvs = FakeDVS("DATA-UCS", [FakePG("existing-pg", vlan=vlan)])
...
def test_rejects_bad_binding_including_deprecated_latebinding(env):
    for bad in ("lateBinding", "static", "", "EPHEMERAL"):
        with pytest.raises(NetworkError, match="binding"):
            create_dvs_portgroup(env.si, "pg", "DATA-UCS", 3832, binding=bad, confirm=True)
...
def test_rejects_vlan_out_of_range(env):
    for bad in (-1, 4095, 99999):
        with pytest.raises(NetworkError, match="vlan_id"):
            create_dvs_portgroup(env.si, "pg", "DATA-UCS", bad, confirm=True)
    assert env.dvs.created_specs == []


def test_rejects_duplicate_name(env):
    with pytest.raises(NetworkError, match="already exists"):
        create_dvs_portgroup(env.si, "existing-pg", "DATA-UCS", 3832, confirm=True)
    assert env.dvs.created_specs == []


def test_unknown_dvs_raises_with_available_list(env):
    with pytest.raises(DvsNotFoundError, match="DATA-UCS"):
        create_dvs_portgroup(env.si, "pg", "nope-vds", 3832, confirm=True)
    assert env.dvs.created_specs == []
...
def test_preview_validates_but_never_writes(env):
    out = create_dvs_portgroup(env.si, "VCF-3832", "DATA-UCS", 3832, binding="ephemeral")
    assert out["action"] == "preview"
    assert out["would_create"]["vlan_id"] == 3832
...
def test_confirm_ephemeral_leaves_numports_unset(env):
    create_dvs_portgroup(env.si, "vcsa-mgmt", "DATA-UCS", 3832, binding="ephemeral", confirm=True)
    spec = env.dvs.created_specs[0]
...
def test_list_reports_vlan_shapes(env, monkeypatch):
    ...
    assert vlans == {"a": "3832", "b": "trunk 100-200", "c": "pvlan 42", "d": "unset"}


def test_list_scoped_to_named_dvs(env):
    out = list_dvs_portgroups(env.si, dvs_name="DATA-UCS")
    assert out["total"] == 1
